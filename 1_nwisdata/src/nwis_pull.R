...
# Currently assuming that an NWIS pull, like a WQP pull, is fastest if we:
...
partition_inventory <- function(inventory_ind, nwis_pull, nwis_codes) {

  nwis_state_codes <- dataRetrieval::stateCd
  ...
  # split up the work into manageably sized partitions (~max size specified in nwis_pull)
  partitions <- bind_rows(lapply(unique(inventory$Constituent), function(constituent) {
    bind_rows(lapply(unique(inventory$StateCode), function(statecode) {
      ...
      # split into multiple nwis pulls - in this case we're defining atomic
      ...
      num_partitions <- ceiling(sum(atomic_groups$NumObs) / nwis_pull$target_pull_size)
  ...
  # prepare nested collections of other possible arguments to include in the readNWISdata calls
  parameter_codes <- bind_rows(lapply(unique(partitions$Constituent), function(constituent) {
    data_frame(Constituent=constituent, CharacteristicNames=nwis_codes$characteristicName[[constituent]]) %>%
      tidyr::nest(-Constituent, .key='Params')
  }))
  site_types <- data_frame(SiteType=nwis_codes$siteType)
...
# per state
plan_nwis_pull <- function(partitions, folders) {
  ...
  partition <- scipiper::create_task_step(
    ...
    command = function(task_name, ...) {
      sprintf("filter_partitions(nwis_pull_partitions, I('%s'))", task_name)
    }
  )
...
create_nwis_pull_makefile <- function(makefile, task_plan) {
  create_task_makefile(
    makefile=makefile, task_plan=task_plan,
    include='1_nwisdata.yml',
    packages=c('dplyr', 'dataRetrieval', 'feather', 'scipiper'),
    file_extensions=c('ind','feather'))
}

get_nwis_data <- function(ind_file, partition, wq_dates) {

  # prepare the argument to pass to readNWISdata
  nwis_args <- list(
  ...
  nwis_dat_time <- system.time({
    nwis_dat <- do.call(dataRetrieval::readNWISdata, nwis_args)
  })
  message(
    "NWIS pull for ", partition$PullTask,
    " took ", nwis_dat_time['elapsed'], " seconds and",
    " returned ", nrow(nwis_dat), " rows")
...
extract_nwis_data <- function(ind_file, local_file, remake_file) {
  nwis_dat <- readRDS(sc_retrieve(local_file, remake_file))

  data_file <- as_data_file(ind_file)
  feather::write_feather(nwis_dat, path=data_file)
  sc_indicate(ind_file, data_file=data_file)
}

extract_nwis_siteinfo <- function(ind_file, local_file, remake_file) {
  nwis_dat <- readRDS(sc_retrieve(local_file, remake_file))
  nwis_dat_siteinfo <- attr(nwis_dat, "siteInfo")

  if(is.null(nwis_dat_siteinfo)) {
    # when nwis_dat is an empty data.frame, attr(nwis_dat, "siteInfo") is NULL
    # write_feather throws an error when it's NULL, so making an empty df instead
    nwis_dat_siteinfo <- data.frame()
  }

  data_file <- as_data_file(ind_file)
  feather::write_feather(nwis_dat_siteinfo, path=data_file)
  sc_indicate(ind_file, data_file=data_file)
}
