...
# Cons for this script - pH, SC, Temp
Con<-'temperature'
StartDate<-'1986-01-01'
EndDate<-'2015-12-31'

# Bad values to remove
if (Con == 'ph') badUnits=c('%')
if (Con == 'SC') badUnits=c('%','m','mg/l','None','',' ')
if (Con == 'temperature') badUnits=c('%','m','mg/l','None','',' ','umho/cm','mS/cm','mg/l      ','umho/cm   ')
    ...
    dat <- feather::read_feather(dfile) %>%
      ...
      filter(!grepl('Quality Control',ActivityTypeCode) & ActivityMediaName %in% c('Water','water','Sediment') &
              ActivityMediaSubdivisionName %in% c('Surface Water',NA,'','Unknown') &
               !ResultMeasure.MeasureUnitCode %in%  badUnits &
  ...
  lapply(site_files, function(dfile) {
    ...
    dat <- feather::read_feather(dfile) %>%
      mutate_at(vars(starts_with('HorizontalAccuracyMeasure.MeasureValue'),starts_with('VerticalAccuracyMeasure.MeasureValue'),
                ...
                starts_with('SourceMapScaleNumeric'),starts_with('DrainageAreaMeasure.MeasureValue'),starts_with('countyCode'),
