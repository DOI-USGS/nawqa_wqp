...
# indicator_files <- dir('1_wqpdata/out/data/nut', pattern='*\\.feather.ind$', full.names=TRUE) %>%
#   grep('(nitrate|phosphorus)', ., value=TRUE) #'(nitrate|phosphorus)'
...
data_files <- dir('1_wqpdata/out/data/Nut', pattern=paste(state,'*', sep=''),full.names = T) %>%
...
combined_data <- bind_rows(
  lapply(data_files, function(dfile) {
    # By State
    cname <- gsub(paste('1_wqpdata/out/data/Nut/',state,'_',sep=''), '', gsub('_00.\\.feather', '', dfile))
    dat <- feather::read_feather(dfile) %>%
      ...
      mutate_at(vars(starts_with('Result'),ends_with('HeightMeasure.MeasureValue'),ends_with('HeightMeasure.MeasureValue'),
                     ...
                     starts_with('SourceMapScaleNumeric')),
                funs(as.character))
  })
...
combined_data %>% group_by(ConstituentGroup) %>% summarise(NumValues=length(ConstituentGroup))
...
write.csv(conList_MR,paste('4_gap_analysis/in/Org/Nut/',stateAb,'_OrgCon_List_MR.csv',sep=''),row.names=F,quote=F)
...
write.table(conList,paste('4_gap_analysis/in/Org/Nut/',stateAb,'_OrgCon_List.csv',sep=''),row.names=F,sep=",") # changed to write.table at DC

# Join constituents to Full agency information
# 9/17 previously had the matching statement as '*(P|p)hos*', this seemed to miss a bunch of names
# Need to re-run
...
customFun  = function(DF) {
  ...
  write.csv(DF,paste0("4_gap_analysis/in/Cons/Nut/",stateAb,"_",unique(DF$ConstituentGroup),".csv"))
  feather::write_feather(DF,paste("4_gap_analysis/in/Cons/Nut/",stateAb,"_",unique(DF$ConstituentGroup),".feather",sep=""))
  return(DF)
}
combined_data2 %>% group_by(ConstituentGroup) %>% do(customFun(.))

## Retrieve siteInfo data and built a siteFile
site_files <- dir('1_wqpdata/out/data/Nut/', pattern=paste(state,'*',sep=""), full.names=TRUE) %>%
...
feather::write_feather(combined_sites,paste("4_gap_analysis/in/Site/Nut/",stateAb,"_sites.feather",sep=""))
write.csv(combined_sites,paste("4_gap_analysis/in/Site/Nut/",stateAb,".csv",sep=""))
...
write.csv(miss_staID,paste("4_gap_analysis/in/Site/Nut/",stateAb,"_MissingSites.csv",sep=""))
